...
assert 'const { contacts, services, allotments, ...baseValues } = values' in frontend and '...baseValues' in frontend, 'hotel edit payload must remove child arrays from the base payload before applying dirty collection rules'
