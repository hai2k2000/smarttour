...
(async () => {
  ...
  const importWriteSearch = await request(manageToken, 'GET', `/suppliers?search=${encodeURIComponent(importWriteCode)}`);
  ...
  const partialWriteSearch = await request(manageToken, 'GET', `/suppliers?search=${encodeURIComponent(partialWriteCode)}`);
  assert(!partialWriteSearch.some((item) => item.supplierCode === partialWriteCode), 'supplier import write must be all-or-nothing');
