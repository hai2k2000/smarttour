...
assert 'Metadata file nhà cung cấp không có object key hợp lệ' in suppliers_service, 'delete must reject corrupted file metadata before deleting DB rows'
assert 'await this.filesService.remove(objectKey)' in suppliers_service, 'delete must remove the validated storage object'
