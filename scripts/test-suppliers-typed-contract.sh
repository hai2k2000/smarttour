...
assert 'function supplierRootPayload(values: SupplierForm, canViewSupplierFinancialFields: boolean)' in frontend and 'function supplierChildPayload(values: SupplierForm)' in frontend and 'metadataPayload(item.metadata)' in frontend, 'typed supplier form must serialize root fields and contacts/services/metadata through stable payload helpers'
assert 'const { contacts, services, taxCode, bankAccountName, bankAccountNumber, bankName, ...baseValues } = values' in frontend, 'typed supplier root payload must strip child collections and financial fields unless finance.payment.view is available'
...
assert 'take: this.listTake(query.take ?? query.limit)' in typed_list_block, 'typed supplier list must apply bounded take'
