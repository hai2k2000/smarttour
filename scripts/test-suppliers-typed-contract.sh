...
assert 'function supplierPayload(values: SupplierForm, canViewSupplierFinancialFields: boolean)' in frontend and 'metadataPayload(item.metadata)' in frontend, 'typed supplier form must serialize contacts/services/metadata through a stable payload helper'
assert 'const { taxCode, bankAccountName, bankAccountNumber, bankName, ...baseValues } = values' in frontend, 'typed supplier payload must strip financial fields unless finance.payment.view is available'
