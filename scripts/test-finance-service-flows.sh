...
async function main() {
  ...
  await rejectsWithStatus(() => finance.createInvoice({ invoiceCode: run + '-BAD-ISO-DATE-INV', customerId: customer.id, invoiceType: 'VAT', issuedDate: '2026-02-31T00:00:00.000Z', tourId: tour.id, items: [{ itemName: 'Bad ISO date invoice', quantity: 1, unitPrice: 1, taxRate: 0 }] }), 400, 'invoice create should reject impossible ISO issued dates');
