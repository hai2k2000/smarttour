...
export class ReportsService {
  ...
  async supplierHistory(supplierId: string, query: ReportQuery, user?: RequestUser) {
    ...
    return this.prisma.operationVoucher.findMany({
      where: this.operationVoucherScopeWhere({ deletedAt: null, supplierId, ...this.dateRange('serviceDate', query.dateFrom, query.dateTo) }, user),
  ...
  private reportDate(value: string) {
    ...
    const datePrefix = dateOnly || /^(\d{4})-(\d{2})-(\d{2})T/.exec(value);
    if (datePrefix) {
