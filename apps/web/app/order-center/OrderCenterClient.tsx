    ...
    <div className="orderPage">
      ...
      <section className="panel orderFilterPanel">
        ...
        <div className="quoteFormGrid orderFilterGrid">
          ...
          <label>Loại don<select value={filters.type || ''} onChange={(event) => setFilter('type', event.target.value)}>{typeOptions.map((item) => <option key={item || 'all'} value={item}>{item || 'Tất cả'}</option>)}</select></label>
          ...
          <label>Ngày về tu<input type="date" value={filters.endFrom || ''} onChange={(event) => setFilter('endFrom', event.target.value)} /></label>
          <label>Ngày về den<input type="date" value={filters.endTo || ''} onChange={(event) => setFilter('endTo', event.target.value)} /></label>
      ...
      <section className="panel listPanel">
        <div className="sectionHeader"><h2>Danh sách don hang tap trung</h2><span>{orders.length} dòng</span></div>
        {orders.length ? (
        ...
        ) : <div className="tableEmptyState">Không có đơn hàng phù hợp bộ lọc.</div>}
      </section>
    </div>
  );
}
