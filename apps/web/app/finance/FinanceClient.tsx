...
export default function FinanceClient() {
  ...
  async function createReceipt(formData: FormData) {
    ...
    const created = await post('/api/finance/receipts', {
      ...
      tourCode: text(formData.get('tourCode')), tourName: text(formData.get('tourName')),
  ...
  async function createPayment(formData: FormData) {
    ...
    const created = await post('/api/finance/payments', {
      ...
      receiverName: text(formData.get('receiverName')), receiverPhone: text(formData.get('receiverPhone')), reason: text(formData.get('reason')), tourCode: text(formData.get('paymentTourCode')),
...
function PaymentForm({ onCreate, onClose, can }: { onCreate: (formData: FormData) => Promise<boolean>; onClose: () => void; can: (permission: string) => boolean }) {
  return <form action={async (formData) => { if (await onCreate(formData)) onClose(); }} className="modalFormStack"><fieldset><legend>Thông tin phiếu chi</legend><div className="formGrid modalFormGrid"><label>Tên phiếu chi<input name="voucherName" required placeholder="Chi thanh toán nhà cung cấp" /></label><label>Loại phiếu chi<select name="voucherType" defaultValue="OTHER">{paymentTypes.map((type) => <option key={type} value={type}>{viStatus(type)}</option>)}</select></label><label>Phương thức thanh toán<select name="paymentMethod" defaultValue="BANK_TRANSFER">{methods.map((method) => <option key={method} value={method}>{viStatus(method)}</option>)}</select></label><label>Ngày thanh toán<input name="paymentDate" type="date" /></label></div></fieldset><fieldset><legend>Người nhận</legend><div className="formGrid modalFormGrid"><label>Người nhận<input name="receiverName" /></label><label>Số điện thoại<input name="receiverPhone" /></label><label>Mã tour<input name="paymentTourCode" /></label><label>Tổng tiền<input name="totalAmount" type="number" min={0} defaultValue={0} /></label><label>Số tiền chi<input name="paymentAmount" type="number" min={1} required defaultValue={0} /></label></div></fieldset><fieldset><legend>Thông tin chuyển khoản</legend><div className="formGrid modalFormGrid"><label>Tên tài khoản<input name="bankAccountName" /></label><label>Số tài khoản<input name="bankAccountNumber" /></label><label>Ngân hàng<input name="bankName" /></label><label>Chi nhánh<input name="branch" /></label><label>Nhân viên phụ trách<input name="assignedStaff" /></label><label>Tệp đính kèm<input name="attachment" type="file" /></label></div></fieldset><fieldset><legend>Diễn giải</legend><label>Lý do chi<textarea name="reason" rows={3} /></label></fieldset><div className="modalActions"><button type="button" className="secondaryButton" onClick={() => { if (window.confirm('Hủy nhập liệu? Dữ liệu chưa lưu sẽ bị mất.')) onClose(); }}>Hủy</button><button type="submit" disabled={!can('finance.payment.create')}>Tạo phiếu chi</button></div></form>;
}
...
function InvoiceForm({ onCreate, onClose, can }: { onCreate: (formData: FormData) => Promise<boolean>; onClose: () => void; can: (permission: string) => boolean }) {
  return <form action={async (formData) => { if (await onCreate(formData)) onClose(); }} className="modalFormStack"><fieldset><legend>Khách hàng và thông tin thuế</legend><div className="formGrid modalFormGrid"><label>Tên khách hàng<input name="customerName" required /></label><label>Số điện thoại<input name="customerPhone" /></label><label>Email<input name="customerEmail" type="email" /></label><label>Mã số thuế<input name="taxCode" /></label><label>Tên đơn vị<input name="companyName" /></label><label>Địa chỉ<input name="companyAddress" /></label></div></fieldset><fieldset><legend>Thông tin hóa đơn</legend><div className="formGrid modalFormGrid"><label>Loại hóa đơn<select name="invoiceType" defaultValue="VAT"><option value="VAT">Hóa đơn VAT</option><option value="NO_VAT">Không VAT</option><option value="ADJUSTMENT">Điều chỉnh</option><option value="REPLACEMENT">Thay thế</option></select></label><label>Ngày xuất<input name="issuedDate" type="date" /></label><label>Mã tour<input name="tourCode" /></label><label>Tên tour<input name="tourName" /></label></div></fieldset><fieldset><legend>Dòng dịch vụ</legend><div className="formGrid modalFormGrid"><label>Dịch vụ<input name="itemName" required defaultValue="Dịch vụ du lịch" /></label><label>Đơn vị tính<input name="unit" defaultValue="gói" /></label><label>Số lượng<input name="quantity" type="number" min={1} required defaultValue={1} /></label><label>Đơn giá<input name="unitPrice" type="number" min={1} required defaultValue={0} /></label><label>Thuế VAT (%)<select name="taxRate" defaultValue="10"><option>0</option><option>5</option><option>8</option><option>10</option></select></label></div></fieldset><fieldset><legend>Tệp và ghi chú</legend><div className="formGrid modalFormGrid"><label className="span2">Tài liệu hóa đơn<input name="attachments" type="file" multiple /></label><label className="span2">Ghi chú<textarea name="note" rows={3} /></label></div></fieldset><div className="modalActions"><button type="button" className="secondaryButton" onClick={() => { if (window.confirm('Hủy nhập liệu? Dữ liệu chưa lưu sẽ bị mất.')) onClose(); }}>Hủy</button><button type="submit" disabled={!can('finance.invoice.create')}>Tạo hóa đơn VAT</button></div></form>;
}
...
function financeLabel(value: unknown) {
  const key = String(value ?? '').trim();
  if (!key) return '-';
  const label = viStatus(key);
  return label === key && key.includes('_') ? 'Khác' : label;
}
function primaryReceiptName(row: Receipt) { return row.receiptName || row.payerName || row.orders?.[0]?.tourName || row.receiptCode || '-'; }
function secondaryReceiptCode(row: Receipt) { return row.receiptCode && row.receiptCode !== primaryReceiptName(row) ? `Mã: ${row.receiptCode}` : ''; }
function primaryPaymentName(row: Payment) { return row.voucherName || row.receiverName || row.voucherCode || '-'; }
function secondaryPaymentCode(row: Payment) { return row.voucherCode && row.voucherCode !== primaryPaymentName(row) ? `Mã: ${row.voucherCode}` : ''; }
function primaryInvoiceName(row: Invoice) { return row.companyName || row.customerName || row.invoiceNumber || row.invoiceCode || '-'; }
function secondaryInvoiceCode(row: Invoice) { return row.invoiceCode && row.invoiceCode !== primaryInvoiceName(row) ? `Mã: ${row.invoiceCode}` : row.invoiceNumber && row.invoiceNumber !== primaryInvoiceName(row) ? `Số: ${row.invoiceNumber}` : ''; }
