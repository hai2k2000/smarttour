...
export const orderConfigs: Record<OrderRouteType, OrderConfig> = {
  'fit-tours': { title: 'Tao Tour khach le FIT', shortTitle: 'Tour FIT', nameLabel: 'Ten tour', codeLabel: 'Ma tour', steps: ['Tinh gia', 'Thong tin Tour', 'Du toan dich vu', 'Dieu hanh dich vu', 'Ban giao', 'Danh gia'] },
  'git-combos': { title: 'Tour GIT/Combo', shortTitle: 'GIT/Combo', nameLabel: 'Lich trinh', codeLabel: 'Ma tour', steps: ['Tinh gia', 'Thong tin Tour', 'Lich trinh', 'DS thanh vien', 'Ban giao', 'Danh gia', 'Dieu khoan'] },
  landtours: { title: 'Don LandTour/Combo', shortTitle: 'LandTour', nameLabel: 'Lich trinh', codeLabel: 'Ma tour', steps: ['Thong tin Tour', 'Lich trinh', 'DS thanh vien', 'Ban giao', 'Danh gia', 'Dieu khoan'] },
  'hotel-bookings': { title: 'Booking phong / Khach san', shortTitle: 'Booking phong', nameLabel: 'Ten tour / dich vu', codeLabel: 'Ma booking', steps: ['Thong tin Booking phong', 'Danh gia dich vu', 'Dieu khoan', 'DS thanh vien'] },
  'single-services': { title: 'Don dich vu le', shortTitle: 'Dich vu le', nameLabel: 'Ten dich vu', codeLabel: 'Ma don', steps: ['Thong tin dich vu', 'Danh gia', 'Dieu khoan', 'DS thanh vien'] },
...
export default function OrdersClient({ type, config, initialOrders }: { type: OrderRouteType; config: OrderConfig; initialOrders: OrderSummary[] }) {
  ...
  async function action(path: string) { if (!editingId) return; const r = await fetch(`${browserApiBase()}/api/orders/${type}/${editingId}/${path}`, { method: 'POST', headers: authJsonHeaders(), body: '{}' }); if (r.ok) { setMessage(`Da thuc hien ${path}`); await reload(); } }
  async function unlockSettlement() { if (!editingId) return; const r = await fetch(`${browserApiBase()}/api/orders/${type}/${editingId}/unlock`, { method: 'POST', headers: authJsonHeaders(), body: JSON.stringify({ actor: 'Operator', reason: 'Mo khoa tu man hinh don hang' }) }); if (r.ok) { setMessage('Da mo khoa quyet toan'); await reload(); } }
  ...
  return (
    <div className="orderPage">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="orderForm">
        ...
        <div className="hotelFormActions"><button type="submit" disabled={isSubmitting || !can('order.manage')}><Save size={17}/> Luu</button><button type="button" className="secondaryButton" disabled={!editingId || !can('order.manage')} onClick={() => action('copy')}><Copy size={17}/> Sao chep</button><button type="button" className="secondaryButton" disabled={!editingId || !can('order.manage')} onClick={() => action('settle')}><Lock size={17}/> Chot quyet toan</button><button type="button" className="secondaryButton" disabled={!editingId || !can('order.manage')} onClick={unlockSettlement}><LockOpen size={17}/> Mo khoa</button><button type="button" className="dangerButton" onClick={closeForm}><X size={17}/> Dong</button></div>
      </form>
