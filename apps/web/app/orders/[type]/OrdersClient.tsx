...
export const orderConfigs: Record<OrderRouteType, OrderConfig> = {
  'fit-tours': { title: 'Tạo Tour khách lẻ FIT', shortTitle: 'Tour FIT', nameLabel: 'Tên tour', codeLabel: 'Mã tour', steps: ['Tính giá', 'Thông tin Tour', 'Dự toán dịch vụ', 'Điều hành dịch vụ', 'Ban giao', 'Đánh giá'] },
  'git-combos': { title: 'Tour GIT/Combo', shortTitle: 'GIT/Combo', nameLabel: 'Lịch trình', codeLabel: 'Mã tour', steps: ['Tính giá', 'Thông tin Tour', 'Lịch trình', 'DS thành viên', 'Ban giao', 'Đánh giá', 'Điều khoản'] },
  landtours: { title: 'Đơn LandTour/Combo', shortTitle: 'LandTour', nameLabel: 'Lịch trình', codeLabel: 'Mã tour', steps: ['Thông tin Tour', 'Lịch trình', 'DS thành viên', 'Ban giao', 'Đánh giá', 'Điều khoản'] },
  'hotel-bookings': { title: 'Booking phòng / Khách sạn', shortTitle: 'Booking phòng', nameLabel: 'Tên tour / dịch vụ', codeLabel: 'Mã booking', steps: ['Thông tin Booking phòng', 'Đánh giá dịch vụ', 'Điều khoản', 'DS thành viên'] },
  'single-services': { title: 'Đơn dịch vụ lẻ', shortTitle: 'Dịch vụ lẻ', nameLabel: 'Tên dịch vụ', codeLabel: 'Mã đơn', steps: ['Thông tin dịch vụ', 'Đánh giá', 'Điều khoản', 'DS thành viên'] },
  'flight-orders': { title: 'Booking vé máy bay', shortTitle: 'Vé máy bay', nameLabel: 'Tên booking / PNR', codeLabel: 'Mã vé', steps: ['Thông tin ve', 'Chặng bay', 'Hành khách', 'Điều khoản', 'Đánh giá'] },
};
...
const defaultValues: OrderForm = {
  systemCode: `DH${Date.now().toString().slice(-6)}`, tourCode: '', holdCode: '', name: '', route: '', marketGroup: 'Khác', bookingDate: '', paymentDate: '', startDate: '', endDate: '', status: 'UPCOMING', tourCategory: 'Khác', currency: 'VND', exchangeRate: 1, createdBy: 'Nhân sự vận hành', createdDate: '', branch: '', customerName: '', customerType: 'Cá nhân', customerPhone: '', customerEmail: '', customerAddress: '', agencyName: '', collaborator: '', operatorOwner: '', adultQty: 0, childQty: 0, infantQty: 0, quantity: 1, roomClass: '', servicePackage: '', transportType: '', pickupPoint: '', dropoffPoint: '', seatTotal: 0, seatHeld: 0, seatSold: 0, allowOverbooking: false, receiveDeadline: '', closeDeadline: '', paidAmount: 0, paidCost: 0, commission: 0, note: '', handoverRequest: '1. Gọi làm quen khách và tạo nhóm Zalo.\n2. Cập nhật điều hành trong nhóm.\n3. Hỗ trợ khách trong hành trình.\n4. Báo cáo phát sinh.', surveyDescription: '', guides: [{ guideName: '', phone: '', language: '', note: '' }], salesItems: [{ ...emptySales }], operationItems: [{ ...emptyOperation }], members: [{ ...emptyMember }], itineraries: [{ dayNo: 1, title: '', content: '', period: '', destination: '', meals: '', hotel: '', restaurant: '', services: '', note: '' }], handoverItems: [{ itemName: 'Danh sách phòng', quantity: 1, note: '' }, { itemName: 'Chương trình tour', quantity: 1, note: '' }], surveyQuestions: [{ question: 'Chất lượng dịch vụ', note: '' }, { question: 'Mức độ hài lòng chung', note: '' }], terms: [{ language: 'VN', terms: '', notes: '' }],
};
...
export default function OrdersClient({ type, config, initialOrders }: { type: OrderRouteType; config: OrderConfig; initialOrders: OrderSummary[] }) {
  ...
  const [financeHistory, setFinanceHistory] = useState({receipts: [], payments: []});
  ...
  const table = useReactTable({
    ...
    columns: useMemo(() => {
      ...
      return [
        helper.display({ id: 'code', header: 'Mã đơn', cell: ({ row }) => <div><strong>{row.original.systemCode}</strong><br /><span className="mutedText">{row.original.tourCode || '-'}</span></div> }),
        helper.accessor('name', { header: config.nameLabel }),
        helper.display({ id: 'dates', header: 'Lịch', cell: ({ row }) => <span>Di: {dateOnly(row.original.startDate) || '-'}<br />Ve: {dateOnly(row.original.endDate) || '-'}</span> }),
        helper.display({ id: 'customer', header: 'Khách hàng', cell: ({ row }) => <span>{row.original.customerName || '-'}<br />{row.original.customerPhone || '-'}</span> }),
        helper.display({ id: 'revenue', header: 'Phan thu', cell: ({ row }) => <span>Tổng: {money(row.original.totalRevenue)}<br />Con: {money(row.original.remainingRevenue)}</span> }),
        helper.display({ id: 'cost', header: 'Phan chi', cell: ({ row }) => <span>Tổng: {money(row.original.totalCost)}<br />Con: {money(row.original.remainingCost)}</span> }),
        helper.accessor('profit', { header: 'Lợi nhuận', cell: (info) => money(info.getValue()) }),
        helper.accessor('status', { header: 'Trạng thái', cell: (info) => <span className={`statusPill status-${info.getValue().toLowerCase()}`}>{viStatus(info.getValue())}</span> }),
        helper.display({ id: 'actions', header: '', cell: ({ row }) => <span className="actionsCell"><button type="button" className="secondaryButton iconTextButton" onClick={() => loadOrder(row.original.id)}><Pencil size={15} /> Sửa</button><button type="button" className="dangerButton iconButton" onClick={() => deleteOrder(row.original.id)}><Trash2 size={14} /></button></span> }),
      ];
    }, [config.nameLabel]),
  ...
  async function loadOrder(id: string) {
    ...
    setFinanceHistory({ receipts: [], payments: [] });
    const [rr, pr] = await Promise.all([
      fetch(`${browserApiBase()}/api/finance/receipts?orderId=${id}`, { headers: authHeaders() }),
      fetch(`${browserApiBase()}/api/finance/payments?orderId=${id}`, { headers: authHeaders() }),
    ]);
    const [rd, pd] = await Promise.all([rr.ok ? rr.json() : {rows:[]}, pr.ok ? pr.json() : {rows:[]}]);
    setFinanceHistory({ receipts: rd.rows || [], payments: pd.rows || [] });
    setShowFinance(true);
  }
  async function onSubmit(data: OrderForm) {
    ...
    if (!r.ok) { setMessage('Không lưu được đơn hàng. Kiểm tra mã đơn và ngày tháng.'); return; }
  ...
  async function deleteOrder(id: string) {
    ...
    const r = await fetch(`${browserApiBase()}/api/orders/${type}/${id}`, { method: 'DELETE', headers: authHeaders() });
    if (r.ok) { setMessage('Da xoa don hang.'); if (editingId === id) closeForm(); await reload(); }
  }
  ...
  async function unlockSettlement() { if (!editingId) return; const r = await fetch(`${browserApiBase()}/api/orders/${type}/${editingId}/unlock`, { method: 'POST', headers: authJsonHeaders(), body: JSON.stringify({ actor: 'Nhân sự vận hành', reason: 'Mở khóa từ màn hình đơn hàng' }) }); if (r.ok) { setMessage('Đã mở khóa quyết toán'); await reload(); } }
  function closeForm() { setEditingId(null); setMessage(''); reset({ ...defaultValues, systemCode: `DH${Date.now().toString().slice(-6)}` }); }
  return (
    <div className="orderPage">
      <PermissionNotice allowed={canAny(['order.view', 'order.manage'])} label="xem và quản lý đơn hàng" />
        ...
        <section className="orderWorkArea">
          <div className="orderMain">
            <section className="panel">
              <div className="sectionHeader"><h2>Thông tin don hang</h2><span>{config.shortTitle}</span></div>
              <div className="quoteFormGrid">
                <label>Mã he thong<input {...register('systemCode')} /></label><label>{config.codeLabel}<input {...register('tourCode')} /></label><label>Mã giu cho<input {...register('holdCode')} /></label><label>{config.nameLabel}<input {...register('name')} /></label><label>Thị trường<input {...register('marketGroup')} /></label>
                <label>Ngày đặt<input type="date" {...register('bookingDate')} /></label><label>Ngày thanh toán<input type="date" {...register('paymentDate')} /></label><label>Ngày đi/CI<input type="date" {...register('startDate')} /></label><label>Ngày về/CO<input type="date" {...register('endDate')} /></label><label>Trạng thái<select {...register('status')}><option value="UPCOMING">UPCOMING</option><option value="RUNNING">RUNNING</option><option value="COMPLETED">COMPLETED</option><option value="CANCELLED">CANCELLED</option></select></label>
                <label>Họ tên khách<input {...register('customerName')} /></label><label>Điện thoại<input {...register('customerPhone')} /></label><label>Email<input type="email" {...register('customerEmail')} /></label><label>CTV<input {...register('collaborator')} /></label><label>NVDH<input {...register('operatorOwner')} /></label>
            ...
            <Rows title="Tour HDV" name="guides" register={register} fieldArray={arrays.guides} emptyRow={{ guideName: '', phone: '', language: '', note: '' }} columns={[['guideName','HDV'],['phone','Điện thoại'],['language','Ngôn ngữ'],['note','Ghi chú']]} />
            <Rows title="Dịch vụ sales / Dự trù tour" name="salesItems" register={register} fieldArray={arrays.salesItems} emptyRow={emptySales} columns={[['serviceType','Loại DV'],['description','Diễn giải'],['quantity','Số lượng','number'],['serviceCount','Số lượt','number'],['unitPrice','Đơn giá','number'],['vat','VAT %','number'],['note','Ghi chú']]} />
            <Rows title="Dịch vụ điều hành / Phần chi" name="operationItems" register={register} fieldArray={arrays.operationItems} emptyRow={emptyOperation} columns={[['serviceType','Loại DV'],['bookingCode','Mã booking'],['serviceDate','Ngày SD','date'],['quantity','Số lượng','number'],['netPrice','Giá NET','number'],['vat','VAT %','number'],['status','Trạng thái'],['note','Ghi chú']]} />
            ...
            <Rows title="Danh sách thành viên" name="members" register={register} fieldArray={arrays.members} emptyRow={emptyMember} columns={[['fullName','Họ tên'],['gender','Giới tính'],['birthday','Ngày sinh','date'],['phone','Điện thoại'],['email','Email'],['identityNumber','CCCD/Passport'],['nationality','Quốc tịch'],['passengerType','Loại khách']]} />
            ...
            <Rows title="Phiếu đánh giá dịch vụ" name="surveyQuestions" register={register} fieldArray={arrays.surveyQuestions} emptyRow={{ question: '', note: '' }} columns={[['question','Câu hỏi'],['note','Ghi chú']]} />
            <Rows title="Điều khoản & Lưu ý" name="terms" register={register} fieldArray={arrays.terms} emptyRow={{ language: 'VN', terms: '', notes: '' }} columns={[['language','Ngôn ngữ'],['terms','Điều khoản','textarea'],['notes','Lưu ý','textarea']]} />
          </div>
      ...
      <section className="panel listPanel"><div className="sectionHeader"><h2>Danh sách {config.shortTitle}</h2><label className="searchBox"><Search size={16}/><input value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="Tìm mã, tên, SĐT..." /></label></div><div className="fitTableWrap"><table className="fitTable orderListTable"><thead>{table.getHeaderGroups().map((g)=><tr key={g.id}>{g.headers.map((h)=><th key={h.id}>{flexRender(h.column.columnDef.header,h.getContext())}</th>)}</tr>)}</thead><tbody>{table.getRowModel().rows.map((r)=><tr key={r.id}>{r.getVisibleCells().map((c)=><td key={c.id}>{flexRender(c.column.columnDef.cell,c.getContext())}</td>)}</tr>)}</tbody></table></div></section>
    </div>
  );
}
