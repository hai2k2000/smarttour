...
by `OPS_FILE_SCAN_TIMEOUT=30s` by default. Its file install/permission commands
are bounded by `OPS_FILE_COMMAND_TIMEOUT=30s` by default.
...
The service units enforce outer `TimeoutStartSec` limits: 10 minutes for
