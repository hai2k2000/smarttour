...
Completed implementation: SmartTour ops systemd services enforce outer `TimeoutStartSec` limits of 2 minutes for healthcheck/host-report, 45 minutes for PostgreSQL backup/restore-drill, and 90 minutes for disaster backup, guarded by `npm run test:ops-log-permissions`.
